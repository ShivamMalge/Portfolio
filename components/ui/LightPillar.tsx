...
const LightPillar = ({
  ...
  if (!webGLSupported) {
    return (
      <div className={`absolute inset-0 w-full h-full flex items-center justify-center bg-black/10 text-muted-foreground text-sm ${className}`} style={{ mixBlendMode: mixBlendMode as any }}>
        WebGL not supported
      </div>
    );
  }

  return <div ref={containerRef} className={`absolute inset-0 w-full h-full ${className}`} style={{ mixBlendMode: mixBlendMode as any }} />;
};
